...
class Heap {
    ...
    public:
        ...
        void insert(T element) {
            ...
            // keep track of new index in case need to swap with parent
            int newIndex = this->tree.size() - 1;

            // parent index checker:
            int parentIndex = floor((newIndex-1)/2);

            //checks value of parent, swap if parent larger than element
            while (this->tree.at(parentIndex) > this->tree.at(newIndex)) {
                std::swap(this->tree[parentIndex], this->tree[newIndex]);
        ...
        void remove(T value) {
            ...
            int currentSize = this->tree.size() - 1;  
            
            // index of value deleting
            int deleting; 
            ...
            // swap end leaf node with one you want to delete
            std::swap(this->tree[currentSize], this->tree[deleting]);

            // now delete the end leaf node (right-most node of bottom level)
            this->tree.pop_back();

            // now to heapify down from index that was deleted
            heapifyDown(deleting);
        ...
        T getMin() {
            // returns value at position 1, as position 0 is dummy value
            return this->tree[1];
        }
};
